var gcObject = {
  ...
  render() {
    ...
    $(
      `<span class='month'>${
        this.options.monthNames[this.pickedDate.getMonth()]
      }</span>`
    ).appendTo(a);
    $(`<span class='year'> ${this.pickedDate.getFullYear()}</span>`).appendTo(
      a
    );
    const o = $(
      `<button type="button" class='prev'>${this.options.prevIcon}</button>`
    );
    o.appendTo(n),
    ...
    const s = $(
      `<button type="button" class='next'>${this.options.nextIcon}</button>`
    );
    s.appendTo(n),
    ...
    p.forEach(function (e) {
      ...
      e.forEach(function (e) {
        ...
        n.addClass(e.class),
          o.appendTo(a),
          g.getFullYear() == e.datejs.getFullYear() &&
            g.getMonth() == e.datejs.getMonth() &&
            g.getDate() == e.datejs.getDate() &&
            a.addClass("today");
        var s = "";
        gcObject.options.events.forEach(function (t) {
          if (
            t.date.getFullYear() == e.datejs.getFullYear() &&
            t.date.getMonth() == e.datejs.getMonth() &&
            t.date.getDate() == e.datejs.getDate()
          ) {
            n.addClass("event");
            var a = $(
              `<div class="gc-event ${t.className}">${t.eventName}</div>`
            );
...
jQuery.fn.calendarGC = function (
  ...
  return (
    (gcObject.options.dayNames = e.dayNames || [
      "Sun",
      "Mon",
      "Tue",
      "Wed",
    ...
    (gcObject.options.dayBegin =
      void 0 === e.dayBegin || null === e.dayBegin ? 1 : e.dayBegin),
    (gcObject.options.monthNames = e.monthNames || [
      "January",
      "February",
      "March",
      "April",
